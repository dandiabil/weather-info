...
L.OWM = L.TileLayer.extend({
  ...
  initialize: function (options) {
    ...
    let tileurl = this.options.baseUrl.replace(
      "{layername}",
      this._owmLayerName
    );
...
L.OWM.Current = L.Layer.extend({
  ...
  initialize: function (options) {
    ...
    this._map = null;
    this._urlTemplate =
      "{WEATHER_API_SERVICE}box/{type}?{appId}cnt=300&format=json&units=metric&bbox={minlon},{minlat},{maxlon},{maxlat},10";
    this._directions = [
      "N",
      "NNE",
      "NE",
  ...
  update: function () {
    ...
    if (data !== null) {
      ...
      const url = this._urlTemplate
        .replace("{WEATHER_API_SERVICE}", WEATHER_API_SERVICE)
        .replace(
          "{appId}",
